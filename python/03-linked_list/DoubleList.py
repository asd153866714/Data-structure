...
def insert():
    ...
    prev = head
    current = head.rlink # head.next -> head.rlink
    while current != head and current.score >= ptr.score: # none -> head
        prev = current
        current = current.rlink # next -> rlink
    ptr.rlink = current
    ptr.llink = prev # single-list 沒有 llink
    prev.rlink = ptr
    current.llink = ptr # 同上
...
def delete():
    ...
    if head.rlink == head: # 無資料顯示錯誤
        ...
        if head != current:
            ...
            current.rlink.llink = prev # llink 指向 prev
            current = None
    ...
    if head.rlink == head:
        ...
        if current != head: # 找到要修改的資料，顯示該筆資料的原始資料
            ...
            while current != head and current.score >= ptr.score:
                prev = current
                current = current.rlink
            ptr.rlink = current # rlink 變更 llink 也要 
            ptr.llink = prev
            prev.rlink = ptr
        else:
            print('\n Student %s not found!\n' % modify_name) # 找不到資料
